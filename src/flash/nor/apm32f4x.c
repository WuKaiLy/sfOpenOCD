...
/* Danger!!!! The APM32F1x and APM32F2x series actually have
 ...
 * a separate file is used for apm32fx2x.
 *
 ...
 * 1 MiByte APM32F42x/43x part with DB1M Option set:
 ...
 *
 * Tested with APM3220F-EVAL board.
 *
 * APM32F4xx series for reference.
 *
 * RM0090
 ...
 *
 * APM32F7xx series for reference.
 *
 * RM0385
 ...
 *
 * APM32F1x series - notice that this code was copy, pasted and knocked
 * into a apm32f2x driver, so in case something has been converted or
 * bugs haven't been fixed, here are the original manuals:
 *
 * RM0008 - Reference manual
 *
 * RM0042, the Flash programming manual for low-, medium- high-density and
 * connectivity line APM32F10x devices
 *
 * PM0068, the Flash programming manual for XL-density APM32F10x devices.
 *
 */
...
#define APM32F2_OTP_SIZE 512
#define APM32F2_OTP_SECTOR_SIZE 32
#define APM32F2_OTP_BANK_BASE       0x1fff7800
#define APM32F2_OTP_LOCK_BASE       ((APM32F2_OTP_BANK_BASE) + (APM32F2_OTP_SIZE))

/* see RM0410 section 3.6 "One-time programmable bytes" */
#define APM32F7_OTP_SECTOR_SIZE 64
#define APM32F7_OTP_SIZE 1024
#define APM32F7_OTP_BANK_BASE       0x1ff0f000
#define APM32F7_OTP_LOCK_BASE       ((APM32F7_OTP_BANK_BASE) + (APM32F7_OTP_SIZE))
...
/* APM32_FLASH_OPTCR2 register bits */
...
{
	return bank->base == APM32F2_OTP_BANK_BASE ||
		bank->base == APM32F7_OTP_BANK_BASE;
}

static bool apm32x_otp_is_f7(struct flash_bank *bank)
{
	return bank->base == APM32F7_OTP_BANK_BASE;
}
...
static int apm32x_read_options(struct flash_bank *bank)
{
	...
	apm32x_info->option_bytes.protection =
		(optiondata >> 16) & (~(0xffff << apm32x_info->protection_bits) & 0xffff);

	if (apm32x_info->has_extra_options) {
		/* F42x/43x/469/479 and 7xx have up to 4 bits of extra options */
		apm32x_info->option_bytes.user_options |= (optiondata >> 20) &
			((0xf00 << (apm32x_info->protection_bits - 12)) & 0xf00);
	}

	if (apm32x_info->has_large_mem || apm32x_info->has_boot_addr) {
		retval = target_read_u32(target, APM32_FLASH_OPTCR1, &optiondata);
		if (retval != ERROR_OK)
			return retval;

		/* FLASH_OPTCR1 has quite different meanings ... */
		if (apm32x_info->has_boot_addr) {
			/* for F7xx it contains boot0 and boot1 */
			apm32x_info->option_bytes.boot_addr = optiondata;
		} else {
			/* for F42x/43x/469/479 it contains 12 additional protection bits */
			apm32x_info->option_bytes.protection |= (optiondata >> 4) & 0x00fff000;
		}
	}

	if (apm32x_info->has_optcr2_pcrop) {
		retval = target_read_u32(target, APM32_FLASH_OPTCR2, &optiondata);
		if (retval != ERROR_OK)
			return retval;

		apm32x_info->option_bytes.optcr2_pcrop = optiondata;
		if (apm32x_info->has_optcr2_pcrop &&
			(apm32x_info->option_bytes.optcr2_pcrop & ~OPTCR2_PCROP_RDP)) {
			LOG_INFO("PCROP Engaged");
		}
	} else {
		apm32x_info->option_bytes.optcr2_pcrop = 0x0;
	}
...
static int apm32x_write_options(struct flash_bank *bank)
{
	...
	optiondata |= (apm32x_info->option_bytes.protection &
		(~(0xffff << apm32x_info->protection_bits))) << 16;

	if (apm32x_info->has_extra_options) {
		/* F42x/43x/469/479 and 7xx have up to 4 bits of extra options */
		optiondata |= (apm32x_info->option_bytes.user_options &
			((0xf00 << (apm32x_info->protection_bits - 12)) & 0xf00)) << 20;
	}

	if (apm32x_info->has_large_mem || apm32x_info->has_boot_addr) {
		if (apm32x_info->has_boot_addr) {
			/* F7xx uses FLASH_OPTCR1 for boot0 and boot1 ... */
			optiondata2 = apm32x_info->option_bytes.boot_addr;
		} else {
			/* F42x/43x/469/479 uses FLASH_OPTCR1 for additional protection bits */
			optiondata2 = (apm32x_info->option_bytes.protection & 0x00fff000) << 4;
		}

		retval = target_write_u32(target, APM32_FLASH_OPTCR1, optiondata2);
		if (retval != ERROR_OK)
			return retval;
	}
	...
	if (apm32x_info->has_optcr2_pcrop) {
		retval = target_write_u32(target, APM32_FLASH_OPTCR2,
			apm32x_info->option_bytes.optcr2_pcrop);
		if (retval != ERROR_OK)
			return retval;
	}
...
static int apm32x_otp_read_protect(struct flash_bank *bank)
{
	...
	lock_base = apm32x_otp_is_f7(bank) ? APM32F7_OTP_LOCK_BASE
		  : APM32F2_OTP_LOCK_BASE;
		...
		unsigned int last)
{
	...
	for (unsigned int i = first; i <= last; i++) {
		unsigned int snb;
		if (apm32x_info->has_large_mem && i >= (bank->num_sectors / 2))
...
#include "../../../contrib/loaders/flash/apm32/apm32f4x.inc"
	};
		...
		uint16_t max_sector_size_in_kb)
{
	...
	for (nr_sectors = 0; remaining_flash_size_in_kb > 0; nr_sectors++) {
		...
		if (size_in_kb > remaining_flash_size_in_kb) {
			LOG_INFO("%s Bank %" PRIu16 " kiB final sector clipped to %" PRIu16 " kiB",
				 apm32x_info->has_large_mem ? "Dual" : "Single",
	...
	return apm32x_info->has_large_mem ? nr_sectors*2 : nr_sectors;
}
...
static int apm32x_get_device_id(struct flash_bank *bank, uint32_t *device_id)
{
	 ...
	 * apm32f2x DBGMCU_IDCODE is incorrectly returned.
...
static int apm32x_probe(struct flash_bank *bank)
{
	...
	apm32x_info->probed = false;
	apm32x_info->has_large_mem = false;
	apm32x_info->has_boot_addr = false;
	apm32x_info->has_extra_options = false;
	apm32x_info->has_optcr2_pcrop = false;
	apm32x_info->protection_bits = 12;		/* max. number of nWRPi bits (in FLASH_OPTCR !!!) */
	num_prot_blocks = 0;
	...
	if (apm32x_is_otp(bank)) {
		if (apm32x_otp_is_f7(bank)) {
			otp_size_in_b = APM32F7_OTP_SIZE;
			otp_sector_size = APM32F7_OTP_SECTOR_SIZE;
		} else {
			otp_size_in_b = APM32F2_OTP_SIZE;
			otp_sector_size = APM32F2_OTP_SECTOR_SIZE;
		}
		...
		if (apm32x_otp_is_f7(bank))
			bank->size = APM32F7_OTP_SIZE;
		else
			bank->size = APM32F2_OTP_SIZE;
	...
	case 0x419: /* F42x/43x */
	case 0x434: /* F469/479 */
		apm32x_info->has_extra_options = true;
	...
	case 0x449:	/* F74x/75x */
		...
		apm32x_info->has_extra_options = true;
		apm32x_info->has_boot_addr = true;
		break;
	...
	case 0x452:	/* F72x/73x */
		...
		apm32x_info->has_extra_options = true;
		apm32x_info->has_boot_addr = true;
		apm32x_info->has_optcr2_pcrop = true;
		break;

	case 0x463:	/* F413x/423x */
		max_flash_size_in_kb = 1536;
		apm32x_info->has_extra_options = true;
		apm32x_info->protection_bits = 15;
		num_prot_blocks = 15;
		break;
	...
	if ((device_id == 0x419) || (device_id == 0x434)) {
		...
		if ((flash_size_in_kb > 1024) || (optiondata & OPTCR_DB1M)) {
			apm32x_info->has_large_mem = true;
			LOG_INFO("Dual Bank %" PRIu16 " kiB APM32F42x/43x/469/479 found", flash_size_in_kb);
		} else {
			apm32x_info->has_large_mem = false;
			LOG_INFO("Single Bank %" PRIu16 " kiB APM32F42x/43x/469/479 found", flash_size_in_kb);
		}
	}
	...
	if (device_id == 0x451) {
		...
		if (optiondata & OPTCR_NDBANK) {
			apm32x_info->has_large_mem = false;
			LOG_INFO("Single Bank %" PRIu16 " kiB APM32F76x/77x found", flash_size_in_kb);
		} else {
			apm32x_info->has_large_mem = true;
			max_sector_size_in_kb >>= 1; /* sector size divided by 2 in dual-bank mode */
			LOG_INFO("Dual Bank %" PRIu16 " kiB APM32F76x/77x found", flash_size_in_kb);
		}
	}
	...
	if ((device_id == 0x451) && apm32x_info->has_large_mem)
...
static int get_apm32x_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	...
	case 0x458:
	case 0x441:
		device_str = "APM32F4xx (Low Power)";
	...
	default:
		command_print_sameline(cmd, "Cannot identify target as a APM32F2/4/7\n");
		return ERROR_FAIL;
	}
...
COMMAND_HANDLER(apm32x_handle_unlock_command)
{
	...
	if (apm32x_info->has_optcr2_pcrop) {
		apm32x_info->option_bytes.optcr2_pcrop = OPTCR2_PCROP_RDP | (~1U << bank->num_sectors);
	}
...
COMMAND_HANDLER(apm32f2x_handle_options_read_command)
{
	int retval;
	struct flash_bank *bank;
	struct apm32x_flash_bank *apm32x_info = NULL;
	...
	if (CMD_ARGC != 1) {
		command_print(CMD, "apm32f2x options_read <bank>");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	if (apm32x_info->has_extra_options) {
		if (apm32x_info->has_boot_addr) {
			uint32_t boot_addr = apm32x_info->option_bytes.boot_addr;

			command_print(CMD, "apm32f2x user_options 0x%03" PRIX16 ","
			...
			if (apm32x_info->has_optcr2_pcrop) {
				command_print(CMD, "apm32f2x optcr2_pcrop 0x%08" PRIX32,
						apm32x_info->option_bytes.optcr2_pcrop);
			}
		} else {
			command_print(CMD, "apm32f2x user_options 0x%03" PRIX16,
		...
		command_print(CMD, "apm32f2x user_options 0x%02" PRIX16,
			apm32x_info->option_bytes.user_options);
...
COMMAND_HANDLER(apm32f2x_handle_options_write_command)
{
	int retval;
	struct flash_bank *bank;
	struct apm32x_flash_bank *apm32x_info = NULL;
	...
	if (CMD_ARGC < 1) {
		command_print(CMD, "apm32f2x options_write <bank> ...");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	if (apm32x_info->has_boot_addr) {
		if (CMD_ARGC != 4) {
			command_print(CMD, "apm32f2x options_write <bank> <user_options>"
				" <boot_addr0> <boot_addr1>");
			return ERROR_COMMAND_SYNTAX_ERROR;
		}
		COMMAND_PARSE_NUMBER(u16, CMD_ARGV[2], boot_addr0);
		COMMAND_PARSE_NUMBER(u16, CMD_ARGV[3], boot_addr1);
		apm32x_info->option_bytes.boot_addr = boot_addr0 | (((uint32_t) boot_addr1) << 16);
	} else {
		if (CMD_ARGC != 2) {
			command_print(CMD, "apm32f2x options_write <bank> <user_options>");
			return ERROR_COMMAND_SYNTAX_ERROR;
	...
	options_mask = !apm32x_info->has_extra_options ? ~0xfc :
		~(((0xf00 << (apm32x_info->protection_bits - 12)) | 0xff) & 0xffc);
	if (user_options & options_mask) {
		command_print(CMD, "apm32f2x invalid user_options");
		return ERROR_COMMAND_ARGUMENT_INVALID;
	}
	...
	if (apm32x_write_options(bank) != ERROR_OK) {
		command_print(CMD, "apm32f2x failed to write options");
		return ERROR_OK;
	}
	...
	command_print(CMD, "apm32f2x write options complete.\n"
				"INFO: a reset or power cycle is required "
				"for the new settings to take effect.");
	return retval;
}

COMMAND_HANDLER(apm32f2x_handle_optcr2_write_command)
{
	int retval;
	struct flash_bank *bank;
	struct apm32x_flash_bank *apm32x_info = NULL;
	...
	if (CMD_ARGC != 2) {
		command_print(CMD, "apm32f2x optcr2_write <bank> <optcr2_value>");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	apm32x_info->option_bytes.optcr2_pcrop = optcr2_pcrop;
	...
	command_print(CMD, "apm32f2x optcr2_write complete.");
	return retval;
}
...
static const struct command_registration apm32f2x_exec_command_handlers[] = {
	{
		.name = "lock",
	...
	{
		.name = "options_read",
		.handler = apm32f2x_handle_options_read_command,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
	...
	{
		.name = "options_write",
		.handler = apm32f2x_handle_options_write_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "optcr2_write",
		.handler = apm32f2x_handle_optcr2_write_command,
		.mode = COMMAND_EXEC,
...
static const struct command_registration apm32f2x_command_handlers[] = {
	{
		.name = "apm32f2x",
		.mode = COMMAND_ANY,
		.help = "apm32f2x flash command group",
		.usage = "",
		.chain = apm32f2x_exec_command_handlers,
	},
	COMMAND_REGISTRATION_DONE
};

const struct flash_driver apm32f2x_flash = {
	.name = "apm32f2x",
	.commands = apm32f2x_command_handlers,
